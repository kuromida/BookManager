...
public class Login_RegisterView implements ActionListener {
	...
	JLabel jlb_text = new JLabel("회원가입");

// String path = "C:\\javaprogramming\\source\\dev_java\\src\\com\\project\\image\\";
	String path = "C:\\image\\";
//yellowLine이미지
	...
	public Login_RegisterView() {
	}

	public Login_RegisterView(boolean isView, String title) {
	...
	public void initDisplay(boolean isView, String title) {
		...
		jlb_juso.setBounds(30, 455, 170, 30);
		jp_new.add(jlb_juso);
		...
		jrb_member.setBounds(155, 504, 79, 30);
		jp_new.add(jrb_member);
		jrb_manager.setBounds(270, 504, 79, 30);
		...
		jlb_grant.setVisible(false);
		jrb_member.setVisible(false);
		jrb_manager.setVisible(false);

	}

	public static void main(String[] args) {
		new Login_RegisterView(true, "회원가입");
	}
	...
	public String[] getMemberText() {// 텍스트필트장에 있는 데이터 읽어오기
		...
		// System.out.println(memText[0]+","+ memText[1]+","+ memText[2]+","
		// + memText[3]+","+ memText[4]+","+ memText[5]+","+ memText[6]);
		return memText;
	}

	// 회원가입 - register()
	public void register() {
		System.out.println("register메소드 호출 완료");
		String memIns[] = getMemberText();

		for (int i = 0; i < memIns.length; i++) {

			System.out.println(memIns[i]);
		}
		MemberVO pmvo = new MemberVO();
		int i = 0;
		pmvo.setMname(memIns[i++]);
		pmvo.setBirthday(memIns[i++]);
		pmvo.setTel(memIns[i++]);
		pmvo.setMemid(memIns[i++]);
		pmvo.setMempw(memIns[i++]);
		pmvo.setAddress(memIns[i++]);
		pmvo.setGender(memIns[i++]);

		pmvo.setCommand("register");
		MemberVO rmvo = loginCtrl.send(pmvo);
		if (rmvo.getResult() == 1) {// 회원가입성공
			JOptionPane.showMessageDialog(jf_new, memIns[0] + "님 환영합니다.", "INFO", JOptionPane.INFORMATION_MESSAGE);
		}
